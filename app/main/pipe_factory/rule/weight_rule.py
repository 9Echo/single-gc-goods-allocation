...
def compose_spec(filtered_item, left_items: list):
    """
    子单拆分重组
    ...
    # 否则使用split_item拆分此处的item一定是一个饱和的子单
        ...
        item, new_item = split_item_spec(filtered_item, filtered_item.weight - new_max_weight)
        if new_item:
...
def compose_optimize(filtered_items: list, left_items: list):
    ...
    for item in filtered_items:
        ...
        if total_weight <= (new_max_weight or g.MAX_WEIGHT):
            ...
            item, new_item = split_item_spec(item, total_weight - (new_max_weight or g.MAX_WEIGHT))
            if new_item:
                candidate_items.append(item)
                left_items.remove(item)
                left_items.insert(0, new_item)
...
def split_item_spec(item, delta_weight):
    """
    拆分超重的订单，将子项全部转为散根计算
