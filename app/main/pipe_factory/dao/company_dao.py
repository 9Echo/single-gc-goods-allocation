...
def write_database(re, now_time):
    """
    :return:
    """
    try:
        ...
        # for re in result:
            # print(re[0][0])
            # print(re[1])
        cursor.execute(sql.format(re[0][0], re[0][1], re[1], now_time))
        print("finish!")
        conn.commit()
    except Exception as e:
        conn.rollback()
...
def update_database_size(re):
    try:
        ...
        conn = db_pool_trans_plan.connection()
        cursor = conn.cursor()
        cursor.execute(sql.format(re[1], re[0][0], re[0][1]))
        conn.commit()
    except Exception as e:
...
if __name__ == '__main__':
    ...
    las = list(last_time)
    data, now_time = get_company_data(las[0])
    print(data, now_time)
